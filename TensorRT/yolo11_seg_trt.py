...
class YoLo11TRT(object):
    ...
    def __init__(self, engine_file_path):
        ...
        self.det_row_output_length = self.seg_c + config.DET_NUM + config.POSE_NUM
    ...
    def post_process(self, output, origin_h, origin_w):
        ...
        result_proto_coef = boxes[:, config.DET_NUM:int(config.DET_NUM + config.SEG_NUM)] if len(boxes) else np.array([])
...
class Colors:
    """
    Diese Klasse verwaltet eine Farbpalette und ermöglicht das Abrufen von Farben
    basierend auf einem Index. Farben können entweder im RGB- oder BGR-Format
    zurückgegeben werden.
    """
    
    def __init__(self):
        """
        Initialisiert die Farben-Palette mit einer vordefinierten Liste von Hexadezimalwerten.
        """
        hex_colors  = (
        ...
        # Konvertiere Hex-Werte in RGB-Tupel und speichere sie in einer Liste
        self.palette: List[Tuple[int, int, int]] = [self.hex2rgb(f'#{c}') for c in hex_colors]
        self.n: int = len(self.palette)  # Anzahl der Farben in der Palette

    def __call__(self, i: int, bgr: bool = False) -> Tuple[int, int, int]:
        """
        Gibt eine Farbe aus der Palette zurück, basierend auf einem Index.

        :param i: Der Index der Farbe in der Palette. Falls der Index größer als die
                  Palettengröße ist, wird er modulo `n` genommen.
        :param bgr: Falls True, wird die Farbe im BGR-Format zurückgegeben (z.B. für OpenCV).
        :return: Ein Tupel mit drei Ganzzahlen, das eine Farbe im RGB- oder BGR-Format darstellt.
        """
        try:
            c = self.palette[int(i) % self.n]  # Modulo-Index, um Überläufe zu verhindern
            return (c[2], c[1], c[0]) if bgr else c  # Falls BGR gewünscht, Reihenfolge umdrehen
        except Exception as e:
            raise ValueError(f"Fehler beim Abrufen der Farbe: {e}")

    @staticmethod
    def hex2rgb(hex_color: str) -> Tuple[int, int, int]:
        """
        Konvertiert eine Hexadezimal-Farbe in ein RGB-Tupel.

        :param hex_color: Hexadezimaler Farbwert als String (z.B. "#FF5733").
        :return: Ein Tupel mit drei Ganzzahlen (R, G, B), jeweils im Bereich 0-255.
        """
        try:
            if not isinstance(hex_color, str) or not hex_color.startswith("#") or len(hex_color) != 7:
                raise ValueError(f"Ungültiges Hex-Format: {hex_color}")

            return tuple(int(hex_color[1 + i:1 + i + 2], 16) for i in (0, 2, 4))
        
        except ValueError as e:
            raise ValueError(f"Fehler bei der Hex-zu-RGB-Konvertierung: {e}")
