...
static inline cv::Mat preprocess_img(cv::Mat& img, int input_w, int input_h) {
    ...
    static inline int read_files_in_dir(const char* p_dir_name, std::vector<std::string>& file_names) {
        // Das Verzeichnis muss mit "\*" (Wildcards) angegeben werden, um alle Dateien zu finden
        std::string search_path = std::string(p_dir_name) + "\\*";
        WIN32_FIND_DATA findFileData;

        // Sucht das erste File im Verzeichnis
        HANDLE hFind = FindFirstFile(search_path.c_str(), &findFileData);
        if (hFind == INVALID_HANDLE_VALUE) {
            // Fehler, wenn das Verzeichnis nicht geöffnet werden kann
            return -1;
        }

        // Liest alle Dateien und Verzeichnisse im angegebenen Verzeichnis
        do {
            // Ignoriert die Verzeichnisse "." und ".."
            if (strcmp(findFileData.cFileName, ".") != 0 && strcmp(findFileData.cFileName, "..") != 0) {
                std::string cur_file_name(findFileData.cFileName);
                file_names.push_back(cur_file_name);
            }
        } while (FindNextFile(hFind, &findFileData) != 0); // Liest den nächsten Dateieintrag
